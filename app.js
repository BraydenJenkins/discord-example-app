...
import { Client } from 'discord.js'; // Import the Client class
  ...
  if (type === InteractionType.APPLICATION_COMMAND) {
    ...
    if (name === 'test') {
      ...
      return res.send({
        ...
        data: {
          ...
          content: 'hello to the world ' + getRandomEmoji(),
        },
      });
    }
    // "challenge" command
    ...
    if(name === 'moveuser') {
      ...
      if(guild) {
        ...
        if (member && channel && channel.type === 'GUILD_VOICE') {
          try {
            await member.voice.setChannel(channel);
            return res.send({
              type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
              data: {
                content: `Moved <@${userId}> to ${channel.name}`,
              },
            });
          } catch (error) {
